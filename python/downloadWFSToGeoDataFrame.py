...
def downloadWFSToGeoDataFrame(url, version, bbox, typename):
    ...
    :typename: key:value pair needed on national georegister website to download certain WFS    
    :returns: GDF: geodataframe with requested data
    """
    MapWFS = WebFeatureService(url=url, version = version)
    response = MapWFS.getfeature(typename=typename, bbox=bbox, maxfeatures=1000, outputFormat='json', startindex=0)
